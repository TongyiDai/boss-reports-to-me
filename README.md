...
这个 skill 把箭头调转过来。它沿着组织架构向上，把你的直属上级、上级的上级、一路到 +4 层，本周在飞书里**你本就有权限看到**的会议纪要、消息发言、文档，收集、去噪、成文，每周五主动推到你面前。你不再追动态，动态自己走过来。
...
三类资产的价值并不对等，周报按**信息密度**排序——**会议纪要 > 消息发言 > 文档**：

<p align="center">
  <img src="assets/evidence-map.png" alt="三类资产按密度排序：会议纪要主口径按参与人、消息按 sender、文档为弱信号，全程只在有权限范围内" />
</p>

| 资产 | 优先级 | 口径 | 关键纪律 |
|---|---|---|---|
| **会议纪要** | 主口径 | 妙记 + 会议记录，**按参与人（participant）捞**，路由到正文取内容 | leader 极少是 owner，纪要多挂在组织者/秘书名下；AI 摘要不当事实 |
| **消息发言** | 次口径 | 按 sender 捞本周消息，关键词留空、靠人+时间召回、分页取全 | 只命中你和 TA 共同所在的群；看不到就是看不到 |
| **文档** | 弱信号 | 按 owner 捞本周新建/改动，内存里按时间精确回裁 | 上级链场景常无产出——leader 不亲自建文档，有则补充、无则不凑 |

> 一句实话：越往上，共同群越少、可见面越小。这个 skill 在 +1 和与你业务交集大的上层价值最高；再往上信号稀疏时，会自动并成一段「上层零星信号」，不硬撑周报。
...
按人名分段，直属在前、更上层在后。每人用**规范字段模板**——段头元信息（姓名·层级·部门·本周证据条数·主场景）+ 固定四段 **总结 → 观点 → 原文 → 下一步** + 统一的原文条目格式。字段顺序固定、缺项留「—」占位，保证每周每人结构一致、可扫读、可对比。像参谋汇报，不像日志转储。
...
### 李硕（直属上级 · People CS）  ·  本周 6 条 · Q3 续费
📌 总结：本周把 Q3 重心从交付明确转向续费。
💡 观点：注意力从「签单」移到「留存」，下半年考核口径可能重排。
💬 原文：
  - 🎙 Q3 复盘会纪要："续费率下滑是下半年最大风险" — Q3 复盘会 8/21 · [纪要](url)
  - 💬 口径统一要求："先把头部客户的续费口径统一" — 项目群 8/22 · [来源](url)
  - 📄 作战文档：《Q3 续费作战》— 8/22 · [链接](url)
→ 下一步：把你手上的数字人方案提前挂到续费指标上。
```

三种风格一句话可切，都用同一套规范字段模板、只变口吻：默认 `chief`（幕僚四段），要更正式 → `sair`（咨询备忘录：情形/分析/洞察/建议），要轻松点 → `humor`（口吻调侃，并用「小王/小李/小刘」这类昵称替代 leader 姓名，仅这份自看周报里用）。
